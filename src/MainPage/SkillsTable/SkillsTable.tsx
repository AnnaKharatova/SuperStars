...
const SkillsTable = () => {
  ...
  useEffect(() => {
    const softList = Array.from(new Set(employers.flatMap((item) =>
      item.skills?.soft_skills?.map(item => item.name)
    )));
    setSoftSkillsList(softList)
    const hardList = Array.from(new Set(employers.flatMap((item) =>
    ...
    <div>
      <div className='skills__header'>
        ...
        <div className='skills__buttons'>
          <button className={`skills__skills-button ${hardSkills ? 'skills__skills-button_active' : ''}`} onClick={handleHardSkills}>Hard skills</button>
          <button className={`skills__skills-button  ${!hardSkills ? 'skills__skills-button_active' : ''}`} onClick={handleSoftSkills}>Soft skills</button>
        </div>
        <div className='skills__buttons skills__buttons-teams'>
          {teams.map(team => (
            <button className={`skills__skills-button ${currentTeam == team ? 'skills__skills-button_active' : ''}`} onClick={() => { handleTeam(team) }}>{team}</button>
          ))}
        </div>
      </div>
      ...
      <div className='table__container'>
        ...
        <table>
          <thead>
            <tr>
              <th className='table__employeer'>Сотрудник
                <button
              ...
              <th className='table__raiting'>Рейтинг
                <button
                  className='table__sort-button'
              ...
              {hardSkills && hardSkillsList.map((skillName, index) => (
              ...
              {!hardSkills && softSkillsList.map((skillName, index) => (
                <th key={index} className='table__skill'>
                  <p>{skillName}</p>
                  <button
                    className='table__sort-button'
            ...
            {employers.map((item, index) => (
              <tr className='table__row' key={index}>
                <td className='employee'>
                  <div>{index + 1}</div>
                  <img className={!item.bus_factor ? 'employee__image' : 'employee__image_true'} src={defaultPhoto} onMouseOver={() => handleMouseOver(index + 'bus-factor')}
                    onMouseOut={handleMouseOut} />
                  {item.bus_factor && <MyTooltip showTooltip={showTooltip === String(index + 'bus-factor')} text={'Bus Factor'} />}
                  <div>
                ...
                <td >
                  ...
                  {hardSkills && item.skills.hard_skills.map((i) => (
                    <td>
                      <div className='employee__score'>
                        <img className='employee__score-progress' src={progressUp} alt='прогресс' />
                        <p className='employee__score-value'>{i.score}</p>
                      </div>
                    </td>
                  ))}
                {!hardSkills && item.skills.soft_skills.map((i) => (
